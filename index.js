...
const API_KEYS = [
  ...
  'AIzaSyAEcnrbEH8uFTp_TSotC0wbyl-eGGP132U'
  // Add more API keys as needed
];

let currentApiKeyIndex = 0;
let apiKeyFailures = new Map(); // Track API key failures

// Add this function to manage API key rotation
function getNextWorkingApiKey() {
  const startIndex = currentApiKeyIndex;
  do {
    const key = API_KEYS[currentApiKeyIndex];
    const failureInfo = apiKeyFailures.get(key) || { count: 0, lastFailure: 0 };
    
    // Reset failure count if it's been more than 5 minutes since last failure
    if (Date.now() - failureInfo.lastFailure > 5 * 60 * 1000) {
      apiKeyFailures.delete(key);
    }
    
    // Use this key if it has less than 3 recent failures
    if (!apiKeyFailures.has(key) || failureInfo.count < 3) {
      return key;
    }
...
// Add this function to handle API key failures
function markApiKeyFailure(key, error) {
  const failureInfo = apiKeyFailures.get(key) || { count: 0, lastFailure: 0 };
  failureInfo.count++;
  failureInfo.lastFailure = Date.now();
  apiKeyFailures.set(key, failureInfo);
  
  // Move to next key
  currentApiKeyIndex = (currentApiKeyIndex + 1) % API_KEYS.length;
  
  console.log(`API key ${key.substring(0, 10)}... failed. Switching to next key. Error: ${error.message}`);
}

const fileManager = new GoogleAIFileManager(getNextWorkingApiKey());
...
client.once('ready', async () => {
  ...
  try {
    ...
    // Set permissions for the settings command
    const guilds = client.guilds.cache.map(guild => guild.id);
    for (const guildId of guilds) {
      const command = await rest.get(Routes.applicationGuildCommands(client.user.id, guildId));
      const settingsCommand = command.find(cmd => cmd.name === 'settings');
      if (settingsCommand) {
        await rest.put(
          Routes.applicationCommandPermissions(client.user.id, guildId, settingsCommand.id),
          {
            body: [
              {
                id: guildId,
                type: 1, // Role
                permission: true,
                // Assuming you have a role ID for admins, replace 'ADMIN_ROLE_ID' with the actual role ID
                roles: ['ADMIN_ROLE_ID']
              }
            ]
          }
...
const ANALYSIS_KEYWORDS = ['analyze', 'review', 'explain', 'summarize', 'tell me about'];
const AFFIRMATIVE_RESPONSES = ['yes', 'sure', 'okay', 'yep', 'yeah', 'ok', 'please', 'go ahead', 'do it'];

// Add this pattern near the top with other constants
const IMAGE_REQUEST_KEYWORDS = ['generate me a picture of', 'create an image of', 'draw', 'sketch', 'illustrate'];

// Add this function to extract YouTube URL from text
function extractYouTubeUrl(text) {
  const urlPattern = /((?:https?:)?\/\/)?((?:www|m)\.)?((?:youtube\.com|youtu.be))(\/(?:[\w\-]+\?v=|embed\/|v\/)?)([\w\-]+)(\S+)?/;
  const match = text.match(urlPattern);
  if (match) {
    if (match[3] === 'youtube.com') {
      const urlParams = new URLSearchParams(match[6] || '');
      const videoId = urlParams.get('v') || match[5];
      return `https://www.youtube.com/watch?v=${videoId}`;
    } else if (match[3] === 'youtu.be') {
      return `https://www.youtube.com/watch?v=${match[5]}`;
    }
  }
  return null;
}

// Add this function to check if text contains analysis request
function containsAnalysisRequest(text) {
  return ANALYSIS_KEYWORDS.some(keyword => text.toLowerCase().includes(keyword));
}

// Add this function to check if text contains an image request
function containsImageRequest(text) {
  return IMAGE_REQUEST_KEYWORDS.some(keyword => text.toLowerCase().includes(keyword));
}
...
client.on('messageCreate', async (message) => {
  try {
    ...
    const mentionPattern = new RegExp(`^<@!?${client.user.id}>(?:\\s+)?(generate|imagine|youtube_transcript)`, 'i');
    const startsWithPattern = /^(generate|imagine|youtube_transcript)/i;
    ...
    if (shouldRespond) {
      ...
      // Handle formal commands
      if (command) {
        if (command[1] === 'youtube_transcript') {
          const url = message.content.split(' ').pop();
          try {
            const processingEmbed = new EmbedBuilder()
              .setColor(hexColour)
              .setTitle('Processing Video')
              .setDescription('Please wait while I analyze the video...');
            const processingMsg = await message.reply({ embeds: [processingEmbed] });
            
            const { text: transcript, explanation, isLong, isExplanationLong } = await generateYouTubeTranscript(url);
            
            const files = [];
            const embeds = [];

            // Handle Analysis
            if (isExplanationLong) {
              const analysisFile = `analysis-${Date.now()}.txt`;
              await writeFile(analysisFile, explanation);
              files.push(new AttachmentBuilder(analysisFile, { name: 'analysis.txt' }));
              
              const explanationEmbed = new EmbedBuilder()
                .setColor(hexColour)
                .setTitle('Video Analysis')
                .setDescription('The analysis was too long to display in Discord. You can find the complete analysis in the attached file.')
                .setFooter({ text: 'Full analysis provided in analysis.txt' });
              embeds.push(explanationEmbed);

              // Clean up file after sending
              setTimeout(() => unlink(analysisFile).catch(console.error), 1000);
            } else {
              const explanationEmbed = new EmbedBuilder()
                .setColor(hexColour)
                .setTitle('Video Analysis')
                .setDescription(explanation)
                .setFooter({ text: 'Analysis by Chachu AI' });
              embeds.push(explanationEmbed);
            }
            ...
            if (isLong) {
              ...
              files.push(new AttachmentBuilder(transcriptFile, { name: 'transcript.txt' }));
              
              const transcriptEmbed = new EmbedBuilder()
                .setColor(hexColour)
                .setTitle('Full Transcript')
                .setDescription('The transcript was too long to display in Discord. You can find the complete transcript in the attached file.')
                .setFooter({ text: 'Full transcript provided in transcript.txt' });
              embeds.push(transcriptEmbed);

              // Clean up file after sending
              setTimeout(() => unlink(transcriptFile).catch(console.error), 1000);
            } else {
              const transcriptEmbed = new EmbedBuilder()
                .setColor(hexColour)
                .setTitle('Video Transcript')
                .setDescription(transcript);
              embeds.push(transcriptEmbed);
            }

            await processingMsg.edit({ embeds, files });
            await addSettingsButton(processingMsg);
        ...
        } else if (command[1] === 'generate' || command[1] === 'imagine') {
        ...
        // Check for YouTube analysis requests
        const youtubeUrl = extractYouTubeUrl(message.content);
        const isAnalysisRequest = containsAnalysisRequest(message.content);
        const isAlwaysRespondChannel = alwaysRespondChannels[message.channelId];

        if (youtubeUrl) {
          if (isAnalysisRequest || isAlwaysRespondChannel) {
            // Direct analysis request
            await handleYouTubeAnalysis(message, youtubeUrl);
          } else if (message.mentions.has(client.user.id)) {
            // Bot was mentioned with a YouTube URL but no explicit analysis request
            const embed = new EmbedBuilder()
              .setColor(hexColour)
              .setTitle('YouTube Video Detected')
              .setDescription('Would you like me to analyze this video for you? Reply with yes/no.');
            
            const botMessage = await message.reply({ embeds: [embed] });
            
            // Store the URL in chat history for context
            updateChatHistory(message.author.id, [
              { role: 'user', content: [{ text: message.content }] },
              { role: 'assistant', content: [{ text: 'Would you like me to analyze this video for you?' }] },
              { role: 'system', content: [{ text: `PENDING_ANALYSIS:${youtubeUrl}` }] }
            ], botMessage.id);
          }
        } else if (containsImageRequest(message.content)) {
          // Handle image generation requests
          const prompt = message.content.split(' ').slice(3).join(' '); // Extract the prompt after the keyword
          await handleImageGeneration(message, prompt);
        } else if (!activeRequests.has(message.author.id)) {
          // Check if this is a response to a pending analysis request
          const history = getHistory(message.author.id);
          const lastSystemMessage = history.findLast(msg => msg.role === 'system' && msg.parts[0].text.startsWith('PENDING_ANALYSIS:'));
          
          if (lastSystemMessage && AFFIRMATIVE_RESPONSES.includes(message.content.toLowerCase())) {
            const url = lastSystemMessage.parts[0].text.split(':')[1];
            await handleYouTubeAnalysis(message, url);
          } else {
...
async function handleCommandInteraction(interaction) {
  ...
  const commandHandlers = {
    ...
    youtube_transcript: async (interaction) => {
      ...
      try {
        ...
        if (explanation.length > 2000) {
          const analysisFile = `analysis-${Date.now()}.txt`;
          await writeFile(analysisFile, explanation);
          const file = new AttachmentBuilder(analysisFile, { name: 'analysis.txt' });
          
          const analysisEmbed = new EmbedBuilder()
            .setColor(hexColour)
            .setTitle('Video Analysis')
            .setDescription('Analysis was too long to display. Check the attached file.')
            .setFooter({ text: 'Analysis by Chachu AI' });
          
          await interaction.editReply({ embeds: [analysisEmbed], files: [file] });
          setTimeout(() => unlink(analysisFile).catch(console.error), 1000);
        } else {
          const analysisEmbed = new EmbedBuilder()
            .setColor(hexColour)
        ...
        // Send transcript as follow-up
        if (transcript.length > 2000) {
          const transcriptFile = `transcript-${Date.now()}.txt`;
          await writeFile(transcriptFile, transcript);
          const file = new AttachmentBuilder(transcriptFile, { name: 'transcript.txt' });
          
          const transcriptEmbed = new EmbedBuilder()
            .setColor(hexColour)
            .setTitle('Video Transcript')
            .setDescription('Transcript was too long to display. Check the attached file.');
          
          await interaction.followUp({ embeds: [transcriptEmbed], files: [file] });
          setTimeout(() => unlink(transcriptFile).catch(console.error), 1000);
        } else {
          const transcriptEmbed = new EmbedBuilder()
            .setColor(hexColour)
...
async function generateVideoExplanation(transcript) {
  let lastError;
  
  // Try each API key until one works
  for (let attempt = 0; attempt < API_KEYS.length; attempt++) {
    try {
      const apiKey = getNextWorkingApiKey();
      ...
      // First, detect if transcript is non-English and translate if needed
      const detectLanguagePrompt = `Analyze this text and tell me if it's in English or another language. If it's not English, specify which language it is. Only respond with the language name or "English". Text: "${transcript.slice(0, 500)}..."`;
      
      const languageResult = await model.generateContent(detectLanguagePrompt);
      const detectedLanguage = languageResult.response.text().toLowerCase();
      
      let translatedTranscript = transcript;
      if (detectedLanguage !== 'english') {
        const translatePrompt = `Translate this ${detectedLanguage} text to English, maintaining any technical terms as is:

${transcript}`;
        const translationResult = await model.generateContent(translatePrompt);
        translatedTranscript = translationResult.response.text();
      }

      const prompt = `As Chachu AI, analyze this video transcript and create a concise, well-structured documentation. This is primarily for Pakistani developers and students looking for quick, accessible learning resources.

Context: This could be a coding tutorial, tech guide, or career advice video. Create documentation that's easy to follow but concise enough that users still benefit from watching the full video.

Please structure your response like a mini-documentation with these sections:
...
📚 Main Topics
- List key topics/concepts covered (with timestamps)
- Highlight technical terms or important tools mentioned
...
Keep the response clear and concise, focusing on practical takeaways. Use bullet points for better readability.
...
${translatedTranscript}`;
      ...
      // Add language notice if transcript was translated
      if (detectedLanguage !== 'english') {
        return `> Note: Original video was in ${detectedLanguage}. This analysis is based on the translated content.\n\n${explanation}`;
      }
      
      return explanation; // If successful, return the result
      ...
      if (error.message.includes('quota') || 
        ...
        markApiKeyFailure(API_KEYS[currentApiKeyIndex], error);
        continue; // Try next key
      }
      
      // If error is not rate-limit related, throw it immediately
      throw error;
    }
  }
  
  // If we get here, all keys failed
  throw new Error(`Failed to generate explanation after trying all API keys. Last error: ${lastError.message}`);
}

async function generateYouTubeTranscript(url) {
    try {
    let videoId;
    ...
    // Get AI explanation
    const explanation = await generateVideoExplanation(formattedTranscript);

    return {
      text: formattedTranscript,
      explanation: explanation,
      isLong: formattedTranscript.length > 3900,
      isExplanationLong: explanation.length > 3900
    };
    } catch (error) {
...
const GRADIENT_BAR = '▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n';
const MAX_EMBED_LENGTH = 3900; // Leave room for gradient bar and formatting
const MAX_TOTAL_EMBED_SIZE = 5800; // Discord's limit is 6000, leave some room for other embed properties

// Updated helper function to handle large content
function formatEmbedDescription(content, addBar = true) {
  const bar = addBar ? GRADIENT_BAR : '';
  
  // If content is within single embed limit, return as is
  ...
  for (const line of lines) {
    if ((currentChunk + line + '\n').length > MAX_EMBED_LENGTH) {
      ...
      if (line.length > MAX_EMBED_LENGTH) {
        const splitPoint = MAX_EMBED_LENGTH - currentChunk.length - 4; // Leave room for "..."
  ...
  // Return first chunk with gradient bar
  return bar + chunks[0];
}

// Add this function to create multiple embeds for long content
function createContentEmbeds(content, title, color, footer = null) {
  const embeds = [];
  let currentContent = content;
  let partNumber = 1;
  const totalParts = Math.ceil(content.length / MAX_EMBED_LENGTH);

  while (currentContent.length > 0) {
    const chunk = formatEmbedDescription(currentContent, partNumber === 1);
    const embed = new EmbedBuilder()
      .setColor(color)
      .setTitle(totalParts > 1 ? `${title} (Part ${partNumber}/${totalParts})` : title)
      .setDescription(chunk);

    if (footer && partNumber === totalParts) {
      embed.setFooter(footer);
    }

    embeds.push(embed);
    currentContent = currentContent.substring(chunk.length);
    partNumber++;

    // If we've reached Discord's total embed limit, stop and create a file instead
    if (embeds.length * chunk.length > MAX_TOTAL_EMBED_SIZE) {
      return { 
        embeds: embeds.slice(0, -1), // Remove last embed that would exceed limit
  ...
  return { embeds, remainingContent: null, needsFile: false };
}

// Modify handleYouTubeAnalysis to use the new functions
async function handleYouTubeAnalysis(message, url) {
  try {
    const processingEmbed = new EmbedBuilder()
      .setColor(hexColour)
      .setTitle('Processing Video')
    ...
    // Send analysis first
    if (explanation.length > 2000) {
      // If analysis is too long, send as file
      const analysisFile = `analysis-${Date.now()}.txt`;
      await writeFile(analysisFile, explanation);
      const file = new AttachmentBuilder(analysisFile, { name: 'analysis.txt' });
      ...
      await message.channel.send({ embeds: [analysisEmbed], files: [file] });
      setTimeout(() => unlink(analysisFile).catch(console.error), 1000);
    } else {
      // Send analysis as regular message with minimal embed
      const analysisEmbed = new EmbedBuilder()
        .setColor(hexColour)
        .setTitle('Video Analysis')
        .setFooter({ text: 'Analysis by Chachu AI' });
    ...
    if (transcript.length > 2000) {
      ...
      await message.channel.send({ embeds: [transcriptEmbed], files: [file] });
      setTimeout(() => unlink(transcriptFile).catch(console.error), 1000);
    } else {
      // Send transcript as regular message with minimal embed
      const transcriptEmbed = new EmbedBuilder()
        .setColor(hexColour)
        .setTitle('Video Transcript');
...
// Also update the initialization of genAI at the top
let genAI = new GoogleGenerativeAI(getNextWorkingApiKey());

// Add a function to refresh genAI instance with new key
function refreshGenAI() {
  try {
    const apiKey = getNextWorkingApiKey();
    genAI = new GoogleGenerativeAI(apiKey);
  } catch (error) {
    console.error('Failed to refresh GenAI instance:', error);
    throw error;
    }
}
...
async function handleImageGeneration(message, prompt) {
  try {
    const processingEmbed = new EmbedBuilder()
      .setColor(hexColour)
      .setTitle('Generating Image')
      .setDescription(`Creating an image for: "${prompt}"`);
    const processingMsg = await message.reply({ embeds: [processingEmbed] });
    ...
    const imageEmbed = new EmbedBuilder()
      ...
      .setImage(imageUrl)
      .setFooter({ text: 'Image generated by Chachu AI' });

    await processingMsg.edit({ embeds: [imageEmbed] });
